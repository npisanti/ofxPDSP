...
- Lazy evaluated DSPs. DSP units are recursively processed in chunks big as the audio buffer size, CPU cycles are spared by not processing Units if the actual state of the system make their results irrelevant and by switching between routines running at audio rate (process every sample) and control rate (once every buffer) on the fly. PDSP is coded optimizing the instruction for this kind of branching and some quick profiling make with valgrind (compiling for debug) show a low I1 cache miss rate (less than 0.3%).
...
You can find a doxygenerated [documentation here.]()
