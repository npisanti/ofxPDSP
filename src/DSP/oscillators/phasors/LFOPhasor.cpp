...
pdsp::Patchable& pdsp::LFOPhasor::set(float freq, float phase_start){
        input_freq.setDefaultValue(freq);
        input_phase_start.setDefaultValue(phase_start); 
        return *this;   
}

pdsp::Patchable& pdsp::LFOPhasor::in_freq(){
    return in("freq");
}
...
pdsp::Patchable& pdsp::LFOPhasor::out_phase(){
    return out("phase");
}

pdsp::Patchable& pdsp::LFOPhasor::out_trig(){
    return out("trig");
}

void pdsp::LFOPhasor::prepareUnit( int expectedBufferSize, double sampleRate) {
    phase = 0.0f;
    incCalculationMultiplier = 1.0f / sampleRate;
}

void pdsp::LFOPhasor::releaseResources () {
    
}

void pdsp::LFOPhasor::process (int bufferSize) noexcept {
...
void pdsp::LFOPhasor::process_audio(const float* freqBuffer, const float* trigBuffer, int bufferSize)noexcept{
...
void pdsp::LFOPhasor::onReset(const int &n, const float &inc, float* &triggerBuffer, const int &bufferSize){
        phase-=1.0f;
