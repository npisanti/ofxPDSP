...
pdsp::CAllPassFilter::CAllPassFilter(const double coefficient)
{
	a=coefficient;
...
pdsp::CAllPassFilter::~CAllPassFilter()
{
};


double pdsp::CAllPassFilter::process(const double input)
{
	//shuffle inputs
	x2=x1;
	x1=x0;
...
pdsp::CAllPassFilterCascade::CAllPassFilterCascade(const double* coefficient, const int N)
{
...
pdsp::CAllPassFilterCascade::~CAllPassFilterCascade()
{
	delete[] allpassfilter;
};


double pdsp::CAllPassFilterCascade::process(const double input)
{
	double output=input;
...
pdsp::CHalfBandFilter::CHalfBandFilter(const int order, const bool steep)
{
	if (steep==true)
	{
	...
	else	//softer slopes, more attenuation and less stopband ripple
	{
		if (order==12)	//rejection=150dB, transition band=0.05
		{
			...
			,0.23094129990840923
			,0.4364942348420355
			,0.6329609551399348 //correct coeff
			,0.80378086794111226
			,0.9599687404800694
			};
...
pdsp::CHalfBandFilter::~CHalfBandFilter()
{
	delete filter_a;
	delete filter_b;
};
...
double pdsp::CHalfBandFilter::process(const double input)
{
